...
public class Game extends JPanel
{
	...
	public void paintComponent(Graphics gr)
	{
		...
		for(int c = 0; c < shard.characters.length; c++)
		{
			...
			if(cha != null)
				g.drawImage(cha.img, (int)(cha.box.x() - ((Circle)cha.box).radius - camera.getX()),
						(int)(cha.box.y() - ((Circle)cha.box).radius - camera.getY()),
						 (int)(((Circle)cha.box).radius*2), (int)(((Circle)cha.box).radius*2), null);
		}
			...
			for(int j = 0; j < shard.units.get(i).size(); j++)
			{
				...
				if(shard.units.get(i).get(j).box instanceof Circle)
					if(unit.img == null)
						...
						g.drawImage(unit.img,
								(int)(unit.box.x() - ((Circle)unit.box).radius - camera.getX()),
								(int)(unit.box.y() - ((Circle)unit.box).radius - camera.getY()),
								 (int)(((Circle)unit.box).radius*2), (int)(((Circle)unit.box).radius*2),
								 null);
				else
					if(unit.img == null)
						g.fillRect((int)(unit.box.x() - camera.getX()),
								(int)(unit.box.y() - camera.getY()), (int)((Rectangle)unit.box).dim.getX(),
								(int)((Rectangle)unit.box).dim.getY());
					else
						g.drawImage(unit.img, (int)(unit.box.x() - camera.getX()),
								(int)(unit.box.y() - camera.getY()), (int)((Rectangle)unit.box).dim.getX(),
								(int)((Rectangle)unit.box).dim.getY(), null);
			}
		...
		for(int x = 0; x < shard.buildings.data.size(); x++)
			for(int y = 0; y < shard.buildings.data.get(x).size(); y++)
				if(shard.buildings.data.get(x).get(y) != null)
					if(shard.buildings.data.get(x).get(y).img == null)
						g.fillRect((int)(x*Tilemap.TILE_SIZE - camera.getX()),
								(int)(y*Tilemap.TILE_SIZE - camera.getY()),
								Tilemap.TILE_SIZE, Tilemap.TILE_SIZE);
					else
						g.drawImage(shard.buildings.data.get(x).get(y).img,
								(int)(x*Tilemap.TILE_SIZE - camera.getX()),
								(int)(y*Tilemap.TILE_SIZE - camera.getY()),
								Tilemap.TILE_SIZE, Tilemap.TILE_SIZE, null);
	}
}
